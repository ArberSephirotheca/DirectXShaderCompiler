...
static constexpr std::array<const char*, 11> kMiniHLSLWaveOps = {
    ...
    "WaveActiveAllEqual", "WaveGetLaneIndex", "WaveGetLaneCount", "WaveIsFirstLane"
};
...
public:
    explicit MiniHLSLMutationGenerator(const std::string& original, std::mt19937& rng) 
...
private:
        ...
        if (pos != std::string::npos) {
            ...
            if (brace_pos != std::string::npos) {
                ...
                while (search_pos < original_.size() && brace_count > 0) {
                    if (original_[search_pos] == '{') brace_count++;
                    else if (original_[search_pos] == '}') brace_count--;
                    search_pos++;
                }
                if (brace_count == 0) {
    ...
    std::string insert_at_random_location(const std::string& code) {
        if (main_body_start_ >= main_body_end_) return "";
    ...
    std::string reorder_commutative_operations() {
        // This is a simplified version - real implementation would parse and reorder AST
        std::string code = "\n    // MiniHLSL: Reordered commutative ops\n"
                          "    uint a = WaveGetLaneIndex();\n"
                          "    uint b = WaveGetLaneCount();\n"
                          "    float result1 = WaveActiveSum(float(a + b));\n"
                          "    float result2 = WaveActiveSum(float(b + a)); // Commutative reorder\n";
...
extern "C" size_t LLVMFuzzerCustomMutator(uint8_t *Data, size_t Size, size_t MaxSize, unsigned int Seed) {
    ...
    if (!contains_wave_intrinsics(input) || contains_forbidden_operations(input)) {
        return 0;
    }
