...
public:
    ...
    const T& unwrap() const { 
        if (!is_success_) throw std::runtime_error("Called unwrap on error Result");
        return std::get<T>(data_);
    }
    
    const E& unwrap_err() const {
        if (is_success_) throw std::runtime_error("Called unwrap_err on ok Result");
    ...
    template<typename F>
    auto and_then(F&& func) -> decltype(func(unwrap())) {
        if (is_ok()) {
            return func(unwrap());
        }
...
enum class ValidationError {
    ...
    UnsynchronizedCompoundRMW,         // Compound read-modify-write without atomics or barriers
...
struct ValidationErrorWithMessage {
    ...
    ValidationErrorWithMessage(ValidationError k, std::string msg, Option<clang::SourceLocation> loc = None<clang::SourceLocation>())
        : kind(k), message(std::move(msg)), location(loc) {}
};

// Rust-style Result type for validation
using ValidationResult = Result<std::monostate, std::vector<ValidationErrorWithMessage>>;
...
public:
    ...
    static ValidationResult err(ValidationError error, const std::string& message, 
...
public:
    ...
    static Box<DeterministicExpressionAnalyzer> create(clang::ASTContext& context) {
    ...
    bool isCompileTimeDeterministic(const clang::Expr* expr) { return is_compile_time_deterministic(expr); }
    ExpressionKind classifyExpression(const clang::Expr* expr) { return classify_expression(expr); }
    ValidationResult validateDeterministicExpression(const clang::Expr* expr) { return validate_deterministic_expression(expr); }
    ...
    bool isLiteralConstant(const clang::Expr* expr) { return is_literal_constant(expr); }
    bool isLaneIndexExpression(const clang::Expr* expr) { return is_lane_index_expression(expr); }
    bool isWavePropertyExpression(const clang::Expr* expr) { return is_wave_property_expression(expr); }
    bool isThreadIndexExpression(const clang::Expr* expr) { return is_thread_index_expression(expr); }
    bool isArithmeticOfDeterministic(const clang::Expr* expr) { return is_arithmetic_of_deterministic(expr); }
    bool isComparisonOfDeterministic(const clang::Expr* expr) { return is_comparison_of_deterministic(expr); }
...
private:
    ...
    // Legacy compatibility methods
    bool analyzeComplexExpression(const clang::Expr* expr) { return analyze_complex_expression(expr); }
    bool analyzeConditionalOperator(const clang::ConditionalOperator* cond) { return analyze_conditional_operator(cond); }
    bool analyzeCastExpression(const clang::CastExpr* cast) { return analyze_cast_expression(cast); }
    bool analyzeArraySubscript(const clang::ArraySubscriptExpr* array) { return analyze_array_subscript(array); }
    bool analyzeInitListExpression(const clang::InitListExpr* initList) { return analyze_init_list_expression(initList); }
    ...
    bool isInDeterministicContext() const { return is_in_deterministic_context(); }
    ...
    // Legacy compatibility
    std::set<std::string> getDependentVariables(const clang::Expr* expr) { return get_dependent_variables(expr); }
    bool areVariablesDeterministic(const std::set<std::string>& variables) { return are_variables_deterministic(variables); }
    ...
    // Legacy compatibility
    bool isDeterministicIntrinsicCall(const clang::CallExpr* call) { return is_deterministic_intrinsic_call(call); }
    bool isDeterministicBinaryOp(const clang::BinaryOperator* op) { return is_deterministic_binary_op(op); }
    bool isDeterministicUnaryOp(const clang::UnaryOperator* op) { return is_deterministic_unary_op(op); }
    bool isDeterministicDeclRef(const clang::DeclRefExpr* ref) { return is_deterministic_decl_ref(ref); }
    bool isDeterministicMemberAccess(const clang::MemberExpr* member) { return is_deterministic_member_access(member); }
    ...
    std::map<std::string, bool>& variableDeterminismCache_ = variable_determinism_cache_;
};
...
public:
    ...
    ValidationResult analyze_statement(const clang::Stmt* stmt, ControlFlowState& state);
    
    // Legacy compatibility
    ValidationResult analyzeFunction(clang::FunctionDecl* func) { return analyze_function(func); }
    ValidationResult analyzeStatement(const clang::Stmt* stmt, ControlFlowState& state) { return analyze_statement(stmt, state); }
    
    // Rust-style control flow construct validation
    ValidationResult validate_deterministic_if(const clang::IfStmt* if_stmt, ControlFlowState& state);
    ValidationResult validate_deterministic_for(const clang::ForStmt* for_stmt, ControlFlowState& state);
    ValidationResult validate_deterministic_while(const clang::WhileStmt* while_stmt, ControlFlowState& state);
    ValidationResult validate_deterministic_switch(const clang::SwitchStmt* switch_stmt, ControlFlowState& state);
    
    // Legacy compatibility
    ValidationResult validateDeterministicIf(const clang::IfStmt* ifStmt, ControlFlowState& state) { return validate_deterministic_if(ifStmt, state); }
    ValidationResult validateDeterministicFor(const clang::ForStmt* forStmt, ControlFlowState& state) { return validate_deterministic_for(forStmt, state); }
    ValidationResult validateDeterministicWhile(const clang::WhileStmt* whileStmt, ControlFlowState& state) { return validate_deterministic_while(whileStmt, state); }
    ValidationResult validateDeterministicSwitch(const clang::SwitchStmt* switchStmt, ControlFlowState& state) { return validate_deterministic_switch(switchStmt, state); }
...
private:
    ...
    ValidationResult analyze_nested_control_flow(const clang::Stmt* stmt, ControlFlowState& state);
    ValidationResult validate_loop_termination(const clang::Expr* condition, const clang::Expr* increment);
    ValidationResult validate_switch_cases(const clang::SwitchStmt* switch_stmt, ControlFlowState& state);
    ValidationResult analyze_break_continue_flow(const clang::Stmt* stmt, ControlFlowState& state);
    
    // Legacy compatibility
    ValidationResult analyzeNestedControlFlow(const clang::Stmt* stmt, ControlFlowState& state) { return analyze_nested_control_flow(stmt, state); }
    ValidationResult validateLoopTermination(const clang::Expr* condition, const clang::Expr* increment) { return validate_loop_termination(condition, increment); }
    ValidationResult validateSwitchCases(const clang::SwitchStmt* switchStmt, ControlFlowState& state) { return validate_switch_cases(switchStmt, state); }
    ValidationResult analyzeBreakContinueFlow(const clang::Stmt* stmt, ControlFlowState& state) { return analyze_break_continue_flow(stmt, state); }
    ...
    // Legacy compatibility
    bool isSimpleDeterministicLoop(const clang::ForStmt* forStmt) { return is_simple_deterministic_loop(forStmt); }
    bool isCountBasedLoop(const clang::ForStmt* forStmt) { return is_count_based_loop(forStmt); }
    bool isLaneIndexBasedBranch(const clang::IfStmt* ifStmt) { return is_lane_index_based_branch(ifStmt); }
    
    // Rust-style flow analysis utilities
    void update_control_flow_state(ControlFlowState& state, const clang::Stmt* stmt);
    bool check_control_flow_consistency(const ControlFlowState& state);
    ValidationResult merge_control_flow_results(const std::vector<ValidationResult>& results);
    
    // Legacy compatibility
    void updateControlFlowState(ControlFlowState& state, const clang::Stmt* stmt) { update_control_flow_state(state, stmt); }
    bool checkControlFlowConsistency(const ControlFlowState& state) { return check_control_flow_consistency(state); }
    ValidationResult mergeControlFlowResults(const std::vector<ValidationResult>& results) { return merge_control_flow_results(results); }
};
...
public:
    ...
    bool has_memory_race_condition(const MemoryOperation& op1, const MemoryOperation& op2);
    
    // Legacy compatibility
    ValidationResult analyzeFunction(clang::FunctionDecl* func) { return analyze_function(func); }
    void collectMemoryOperations(clang::FunctionDecl* func) { collect_memory_operations(func); }
    bool hasDisjointWrites() { return has_disjoint_writes(); }
    bool hasOnlyCommutativeOperations() { return has_only_commutative_operations(); }
    ...
    bool are_conflicting_accesses(const MemoryOperation& op1, const MemoryOperation& op2);
    ...
    bool is_simple_rmw_operation(const MemoryOperation& read, const MemoryOperation& write);
    bool requires_atomic_rmw(const MemoryOperation& read, const MemoryOperation& write);
    bool has_barrier_synchronization(const MemoryOperation& op1, const MemoryOperation& op2);
    
    // Legacy compatibility methods
    ValidationResult validateDataRaceFreedom() { return validate_data_race_freedom(); }
    bool hasDataRace(const MemoryOperation& op1, const MemoryOperation& op2) { return has_data_race(op1, op2); }
    bool areConflictingAccesses(const MemoryOperation& op1, const MemoryOperation& op2) { return are_conflicting_accesses(op1, op2); }
    bool areSynchronized(const MemoryOperation& op1, const MemoryOperation& op2) { return are_synchronized(op1, op2); }
    ValidationResult validateHybridRMWApproach() { return validate_hybrid_rmw_approach(); }
    bool isSimpleRMWOperation(const MemoryOperation& read, const MemoryOperation& write) { return is_simple_rmw_operation(read, write); }
    bool requiresAtomicRMW(const MemoryOperation& read, const MemoryOperation& write) { return requires_atomic_rmw(read, write); }
    bool hasBarrierSynchronization(const MemoryOperation& op1, const MemoryOperation& op2) { return has_barrier_synchronization(op1, op2); }
...
private:
    ...
    ValidationResult analyzeSharedMemoryUsage() { return analyze_shared_memory_usage(); }
    ValidationResult validateAtomicOperations() { return validate_atomic_operations(); }
    ValidationResult checkMemoryAccessPatterns() { return check_memory_access_patterns(); }
    
    // Rust-style sophisticated alias analysis
    bool could_alias(const clang::Expr* addr1, const clang::Expr* addr2);
    bool analyze_address_expressions(const clang::Expr* addr1, const clang::Expr* addr2);
    bool is_disjoint_lane_based_access(const clang::Expr* addr1, const clang::Expr* addr2);
    bool is_constant_offset(const clang::Expr* expr, int64_t& offset);
    
    // Legacy compatibility
    bool couldAlias(const clang::Expr* addr1, const clang::Expr* addr2) { return could_alias(addr1, addr2); }
    bool analyzeAddressExpressions(const clang::Expr* addr1, const clang::Expr* addr2) { return analyze_address_expressions(addr1, addr2); }
    bool isDisjointLaneBasedAccess(const clang::Expr* addr1, const clang::Expr* addr2) { return is_disjoint_lane_based_access(addr1, addr2); }
    bool isConstantOffset(const clang::Expr* expr, int64_t& offset) { return is_constant_offset(expr, offset); }
    ...
    // Legacy compatibility
    MemoryAccessPattern classifyMemoryAccess(const MemoryOperation& op) { return classify_memory_access(op); }
    bool isCommutativeMemoryOperation(const MemoryOperation& op) { return is_commutative_memory_operation(op); }
    bool requiresAtomicity(const MemoryOperation& op) { return requires_atomicity(op); }
    ...
    // Legacy compatibility
    std::set<std::string> getAliasSet(const std::string& resourceName) { return get_alias_set(resourceName); }
    void buildMemoryDependencyGraph() { build_memory_dependency_graph(); }
    ValidationResult analyzeMemoryDependencies() { return analyze_memory_dependencies(); }
...
public:
    ...
    ValidationResult validate_wave_call(const clang::CallExpr* call, 
    ...
    // Legacy compatibility
    ValidationResult validateWaveCall(const clang::CallExpr* call, const ControlFlowAnalyzer::ControlFlowState& cfState) {
        return validate_wave_call(call, cfState);
    }
    bool isWaveOperation(const clang::CallExpr* call) { return is_wave_operation(call); }
    bool isOrderIndependentWaveOp(const std::string& funcName) { return is_order_independent_wave_op(funcName); }
    bool isOrderDependentWaveOp(const std::string& funcName) { return is_order_dependent_wave_op(funcName); }
    bool requiresFullParticipation(const std::string& funcName) { return requires_full_participation(funcName); }
    
private:
    // Rust-style advanced wave operation analysis
    ValidationResult analyze_wave_participation(const clang::CallExpr* call, 
                                               const ControlFlowAnalyzer::ControlFlowState& cf_state);
    ValidationResult validate_wave_arguments(const clang::CallExpr* call, const std::string& func_name);
    ValidationResult check_wave_operation_context(const clang::CallExpr* call);
    
    // Legacy compatibility
    ValidationResult analyzeWaveParticipation(const clang::CallExpr* call, const ControlFlowAnalyzer::ControlFlowState& cfState) {
    ...
    // Legacy compatibility
    WaveOperationType classifyWaveOperation(const std::string& funcName) { return classify_wave_operation(funcName); }
    bool isReductionOperation(const std::string& funcName) { return is_reduction_operation(funcName); }
    bool isBroadcastOperation(const std::string& funcName) { return is_broadcast_operation(funcName); }
    bool isQueryOperation(const std::string& funcName) { return is_query_operation(funcName); }
    
    // Rust-style context analysis
    bool is_in_uniform_control_flow(const ControlFlowAnalyzer::ControlFlowState& cf_state);
    bool is_in_divergent_control_flow(const ControlFlowAnalyzer::ControlFlowState& cf_state);
    int calculate_divergence_level(const ControlFlowAnalyzer::ControlFlowState& cf_state);
    
    // Legacy compatibility
    bool isInUniformControlFlow(const ControlFlowAnalyzer::ControlFlowState& cfState) { return is_in_uniform_control_flow(cfState); }
    bool isInDivergentControlFlow(const ControlFlowAnalyzer::ControlFlowState& cfState) { return is_in_divergent_control_flow(cfState); }
    int calculateDivergenceLevel(const ControlFlowAnalyzer::ControlFlowState& cfState) { return calculate_divergence_level(cfState); }
...
public:
    ...
    // Legacy compatibility
    ValidationResult validateProgram(const Program* program) { return validate_program(program); }
    ValidationResult validateFunction(const Function* func) { return validate_function(func); }
    ValidationResult validateSource(const std::string& hlslSource) { return validate_source(hlslSource); }
    
    // Rust-style AST-based methods
    ValidationResult validate_ast(clang::TranslationUnitDecl* tu, clang::ASTContext& context);
    ValidationResult validate_source_with_full_ast(const std::string& hlsl_source, 
                                                   const std::string& filename = "shader.hlsl");
    
    // Legacy compatibility
    ValidationResult validateAST(clang::TranslationUnitDecl* tu, clang::ASTContext& context) { return validate_ast(tu, context); }
    ValidationResult validateSourceWithFullAST(const std::string& hlslSource, const std::string& filename = "shader.hlsl") {
    ...
    FormalProofMapping map_to_formal_proof(const ValidationResult& result, const Program* program);
    
    // Legacy compatibility
    std::string generateFormalProofAlignment(const Program* program) { return generate_formal_proof_alignment(program); }
    FormalProofMapping mapToFormalProof(const ValidationResult& result, const Program* program) { return map_to_formal_proof(result, program); }
    
    // Rust-style advanced analysis capabilities
    ValidationResult perform_full_static_analysis(clang::TranslationUnitDecl* tu, clang::ASTContext& context);
    ValidationResult analyze_order_independence(clang::TranslationUnitDecl* tu, clang::ASTContext& context);
    ValidationResult generate_optimization_suggestions(clang::TranslationUnitDecl* tu, clang::ASTContext& context);
...
private:
    ...
    ValidationResult validate_forbidden_wave_operations(const std::string& hlsl_source);
    ValidationResult validate_control_flow_patterns(const std::string& hlsl_source);
    ValidationResult validate_wave_operation_usage(const std::string& hlsl_source);
    ValidationResult validate_memory_safety_patterns(const std::string& hlsl_source);
    
    // Rust-style AST parsing and setup
    std::pair<Box<clang::ASTContext>, clang::TranslationUnitDecl*> 
    parse_hlsl_with_complete_ast(const std::string& source, const std::string& filename);
    ...
    std::unique_ptr<clang::CompilerInstance> setupCompleteCompiler() { return setup_complete_compiler(); }
    
    // Rust-style validation workflow
    ValidationResult run_complete_validation(clang::TranslationUnitDecl* tu, clang::ASTContext& context);
    ValidationResult validate_all_constraints(clang::TranslationUnitDecl* tu, clang::ASTContext& context);
    ...
    ValidationResult generateComprehensiveReport(const ValidationResult& result, clang::TranslationUnitDecl* tu, clang::ASTContext& context) {
    ...
    ValidationResult coordinate_analysis(clang::TranslationUnitDecl* tu, clang::ASTContext& context);
    ValidationResult consolidate_results(const std::vector<ValidationResult>& results);
    
    // Legacy compatibility
    void initializeAnalyzers(clang::ASTContext& context) { initialize_analyzers(context); }
...
public:
    ...
    // Legacy compatibility
    static std::unique_ptr<MiniHLSLValidator> createValidator() { return create_validator(); }
    static std::unique_ptr<MiniHLSLValidator> createMiniHLSLValidator() { return create_mini_hlsl_validator(); }
};
